        ...
        for player in game.players:
            ...
            if 'm' in player:
                if pac_controller is None:
                    ...
                    scores = [pac_controller.score(state) for state in s_primes]
                    
                    # 2a TODO: Assign index of state with the best score to selected_action_idx.
                    selected_action_idx = max(enumerate(scores), key=lambda x: x[1])[0]
...
#                     print(actions)
            ...
            # Select Ghost action(s) using provided strategy.
                ...
                if ghost_controller is None:
                    ...
                    scores = [ghost_controller.score(state, ghost_id=player) for state in s_primes]
                    selected_action_idx = max(enumerate(scores), key=lambda x: x[1])[0]
